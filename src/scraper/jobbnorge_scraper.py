"""
Jobbnorge.no scraper — searches job postings by keyword.
Strong in public sector and academia positions in Norway.
...
JOBBNORGE_BASE_URL = "https://www.jobbnorge.no"
JOBBNORGE_SEARCH_URL = f"{JOBBNORGE_BASE_URL}/search/en"


def _build_search_url(keyword: str, page: int = 1) -> str:
    encoded = keyword.replace(" ", "+")
    url = f"{JOBBNORGE_SEARCH_URL}?q={encoded}"
    if page > 1:
        url += f"&page={page}"
    return url
...
def _extract_job_id(url: str) -> str:
    ...
    match = re.search(r"/(?:en|nn)/available-jobs/job/(\d+)", url)
...
def _parse_listing_page(page: Page, keyword: str) -> list[dict]:
    ...
    logger.debug("Parsed %d cards on Jobbnorge for '%s'", len(results), keyword)
    return results
...
def _scrape_pages(page: Page, keyword: str, max_pages: int, known_ids: set = None) -> Generator[dict, None, None]:
    for page_num in range(1, max_pages + 1):
        ...
        try:
            ...
            try:
                page.click("button:has-text('Godta'), button:has-text('Accept')", timeout=2000)
            ...
            logger.warning("Timeout on jobbnorge page %d for '%s'", page_num, keyword)
            break
        except Exception as exc:
            logger.error("Error scraping jobbnorge page %d: %s", page_num, exc)
            break
